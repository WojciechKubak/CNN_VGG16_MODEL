...
Build a new **dataframe** object with the first column being the previously created list of file names. Then, starting from the top, create new columns in which one is the full path of the file and the other is the extracted class name. We remove the 'image' column, which is redundant at this point. In the last steps, shuffle the dataset and reset its index.
...
The next step will be to divide the collection into 3 parts in the proportion of 70% - 15% - 15%, constituting in turn the training collection, the validation collection and the test collection. To achieve this first read the total number of available samples.
...
Using the API, we load the model without the top layer, set the weights to untrainable and add a new input with the appropriate shape. Then, when the **feature extractor** is ready, add a flatten layer and then 2 fully connected layers followed by the softmax on the output. 
