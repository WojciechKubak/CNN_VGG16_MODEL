...
Now by plotting history.history we can see how the error rate and accuracy for each epoch in the training and validation set evolved.
...
In order to evaluate our model, another instance of it will be created, then we will load the best weights obtained by the ModelCheckpoint callback stored in the path. 
...
At this level, we can evalue our model with samples from the test set that have not been used before.
...
Mając już przekształcone wszystkie próbki możemy utworzyć macierz pomyłek, dodatkowe utworzymy sobie listę zawierającą unikalne labele występujące w zbiorze danych.  
